...
export default function AddTeacherPage() {
  ...
  const addTeacherHandle = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/crud/teacher`, {
        method: "POST",
  ...
  return (
    <form className="bg-white p-8 rounded-lg shadow-lg flex flex-col min-h-full gap-4 justify-between" ref={formref} onSubmit={addTeacherHandle}>
      <div>
        ...
        <label className="block text-base font-medium text-[#4A4E69]">
          ...
          <input
            type="email"
            className=" bg-gray-200 text-sm mt-1 p-2 block w-96  border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
            onChange={(event) => {
